// Волновой алгоритм — это метод поиска кратчайшего пути в графе, который часто используется для поиска пути на сетке, такой как карта или лабиринт. Этот алгоритм работает по принципу волны, постепенно "распространяясь" от начальной точки в соседние вершины.

// ### Описание Алгоритма

// - **Инициализация**: Создайте очередь для хранения вершин, которые должны быть обработаны. Также создайте двумерный массив для хранения расстояний от стартовой точки до каждой точки.

// - **Фаза обработки**: Извлекайте вершины из очереди, проверяйте их соседей и обновляйте расстояния до них, добавляя их в очередь, если они не были посещены.

// - **Завершение**: Процесс продолжается до тех пор, пока не будут обработаны все reachable вершины или пока не достигнется целевая вершина.

// ### Пример Реализации на C

// Вот пример реализации волнового алгоритма на языке C, предполагающий, что граф представлен в виде матрицы смежности:
